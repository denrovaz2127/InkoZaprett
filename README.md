...
Автономное средство, которое не требует подключения каких-либо сторонних серверов. Может помочь
обойти замедление сайтов HTTP(S), сигнатурный анализ TCP и UDP протоколов, например. 
Может использоваться для частичной прозрачной обфускации протоколов.

Поддерживаются Windows. MacOS не поддерживается и вряд ли будет по техническим причинам.
